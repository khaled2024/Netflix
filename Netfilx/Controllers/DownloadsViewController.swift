...
class DownloadsViewController: UIViewController {
    ...
    //MARK: - private func
    private func fetchLocalStogare(){
...
extension DownloadsViewController: UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(with: UpcomingTitle(posterUrl: title.poster_path ?? "" , titleName: title.original_title ?? title.original_name ?? "UnKnown name"))
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        switch editingStyle{
        case .delete :
            DataPersistenceManager.shared.deleteWith(model: titles[indexPath.row]) { result in
                switch result{
                case .success():
